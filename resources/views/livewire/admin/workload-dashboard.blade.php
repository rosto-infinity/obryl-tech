<x-layouts.app :title="__('Gestion de Charge')">
      <div class="space-y-6">
                    {{-- Header --}}
                    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex space-x-3">
                                <button wire:click="handleOverload" wire:loading.attr="disabled"
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $activeDevelopers }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ $overloadedDevelopers }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            <div class="flex items-center">
                                <div class="p-3 bg-yellow-100 rounded-full">
                                    <span
                                        class="w-6 h-6 flex items-center justify-center text-yellow-600 font-bold text-sm">⏰</span>
                                </div>
                                <div class="ml-4">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            <div class="flex items-center">
                                <div class="p-3 bg-purple-100 rounded-full">
                                    <span
                                        class="w-6 h-6 flex items-center justify-center text-purple-600 font-bold text-sm">💰</span>
                                </div>
                                <div class="ml-4">
                                    <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Commissions Mensuelles</p>
                                    <p class="text-2xl font-bold text-gray-900 dark:text-white">
                                        {{ number_format($monthlyCommissions, 0, ',', ' ') }} XAF</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            ...
                            <div class="space-y-4">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-bold">{{ $workloadStats['total_developers'] }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Disponibles</span>
                                    <span class="font-bold text-green-600">{{ $workloadStats['available'] }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Occupés</span>
                                    <span class="font-bold text-yellow-600">{{ $workloadStats['busy'] }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Surchargés</span>
                                    <span class="font-bold text-red-600">{{ $workloadStats['overloaded'] }}</span>
                                </div>
                                <div class="flex justify-between items-center">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Charge Moyenne</span>
                                    <span class="font-bold">{{ number_format($workloadStats['avg_workload'], 1) }}%</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                            ...
                            @if (empty($recentReassignments))
                                <p class="text-gray-500 text-sm">Aucune réassignation récente</p>
                            @else
                                <div class="space-y-3">
                                    @foreach ($recentReassignments as $reassignment)
                                        <div class="border-l-4 border-gray-200 pl-4">
                                            <div class="flex items-start justify-between">
                                                <div>
                                                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                                                        <strong>{{ $reassignment['project_title'] }}</strong>
                                                    </p>
                                                    <p class="text-xs text-gray-600 dark:text-gray-400">
                                                        De: {{ $reassignment['previous_developer'] }} →
                                                        À: {{ $reassignment['new_developer'] }}
                                                    </p>
                                                    <p class="text-xs text-gray-500">
                                                        {{ \Carbon\Carbon::parse($reassignment['created_at'])->diffForHumans() }}
                                                    </p>
                                                </div>
                                                <div class="text-xs text-gray-500">
                                                    {{ \Carbon\Carbon::parse($reassignment['created_at'])->format('d/m H:i') }}
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
