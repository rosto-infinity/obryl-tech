<div class="bg-white min-h-screen pb-20">
    <!-- Hero Section -->
    <div class="relative w-full h-[60vh] overflow-hidden bg-gray-50 border-b border-gray-100">
        <img 
        ...
        <div class="absolute inset-0 flex items-end">
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 w-full">
                <!-- Back Button -->
                <a href="{{ route('blog.index') }}" wire:navigate class="inline-flex items-center text-gray-400 hover:text-primary mb-8 transition-all group">
                    <div class="w-10 h-10 rounded-full border border-gray-200 flex items-center justify-center mr-4 group-hover:border-primary/50 group-hover:bg-primary/5 transition-all">
                ...
                @if($article->category)
                    <div class="mb-6">
                        <span class="inline-flex items-center px-5 py-2 rounded-2xl bg-primary/10 backdrop-blur-xl text-primary text-[10px] font-black border border-secondary/20 uppercase tracking-[0.2em] shadow-sm">
                ...
                <h1 class="text-4xl md:text-7xl font-black text-gray-900 leading-[1.1] mb-8 tracking-tighter max-w-4xl">
                    {{ $article->title }}
                </h1>

                <div class="flex flex-wrap items-center gap-8 text-gray-500 text-[10px] font-black uppercase tracking-[0.2em]">
                    <div class="flex items-center group cursor-pointer">
                        <div class="w-12 h-12 rounded-2xl bg-white border border-gray-200 flex items-center justify-center mr-4 overflow-hidden shadow-sm transform group-hover:rotate-6 transition-transform">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-20 grid grid-cols-1 lg:grid-cols-4 gap-16">
        ...
        <div class="hidden lg:block lg:col-span-1 space-y-16">
            <div class="sticky top-24 space-y-12">
                <!-- Stats Box -->
                <div class="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl shadow-gray-100/50 overflow-hidden relative group">
                    ...
                    <div class="grid grid-cols-1 gap-6 relative z-10">
                        <div class="p-6 bg-gray-50 rounded-3xl border border-gray-100 flex flex-col items-center">
                ...
                <div class="space-y-6 px-4">
                    <h3 class="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em]">Expertises associées</h3>
                    <div class="flex flex-wrap gap-2">
                        @foreach($article->tags ?? [] as $tag)
                            <a href="#" class="px-4 py-2 bg-gray-50 text-gray-500 text-[10px] font-bold rounded-xl border border-gray-100 hover:border-primary hover:text-primary transition-all uppercase tracking-widest">
                                #{{ $tag }}
                            </a>
                        @endforeach
                    </div>
        ...
        <div class="lg:col-span-3 space-y-24">
            <!-- Prose Content -->
            <article class="prose prose-2xl max-w-none prose-primary prose-headings:font-black prose-headings:tracking-tighter prose-headings:text-gray-900 prose-p:text-gray-600 prose-p:leading-relaxed prose-a:text-primary prose-a:font-bold prose-a:no-underline hover:prose-a:underline prose-blockquote:border-primary prose-blockquote:bg-gray-50 prose-blockquote:rounded-3xl prose-blockquote:p-10 prose-img:rounded-[3rem] prose-pre:bg-gray-900 prose-pre:rounded-[2rem] prose-pre:border prose-pre:border-gray-800 prose-strong:text-gray-900 prose-li:text-gray-600">
            ...
            <section id="comments" class="space-y-16">
                <div class="flex items-center justify-between">
                    <h2 class="text-4xl font-black text-gray-900 tracking-tighter">Réactions <span class="text-primary">({{ count(array_filter($article->comments ?? [], fn($c) => $c['status'] === 'approved')) }})</span></h2>
                </div>
                ...
                <div class="space-y-8">
                    @forelse(array_filter($article->comments ?? [], fn($c) => $c['status'] === 'approved') as $comment)
                        <div class="bg-white p-8 rounded-[2.5rem] border border-gray-100 hover:border-primary/20 transition-all shadow-sm">
                            <div class="flex items-center justify-between mb-6">
                                <div class="flex items-center">
                                    <div class="w-12 h-12 rounded-2xl bg-gray-50 border border-gray-100 flex items-center justify-center mr-4">
                                        <span class="text-primary text-sm font-black uppercase">{{ substr($comment['user_name'], 0, 1) }}</span>
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="text-gray-900 font-black text-sm uppercase tracking-wider">{{ $comment['user_name'] }}</span>
                                        <span class="text-[9px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-1">
            ...
            @if(count($similarArticles) > 0)
                <section class="space-y-12">
                    <h2 class="text-4xl font-black text-gray-900 tracking-tighter">Continuer <span class="text-primary italic">l'exploration</span></h2>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                        @foreach($similarArticles as $similar)
                            <a href="{{ route('blog.show', $similar->slug) }}" wire:navigate class="group block bg-gray-50 rounded-[2.5rem] overflow-hidden border border-gray-100 hover:border-primary/20 hover:bg-white transition-all p-6 shadow-sm hover:shadow-xl">
                                ...
                                <h3 class="text-gray-900 font-bold leading-tight group-hover:text-primary transition-colors line-clamp-2 text-lg tracking-tight">
                                    {{ $similar->title }}
                                </h3>
                            </a>
                        @endforeach
